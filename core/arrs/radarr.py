...
class RadarrClient(AbstractArrClient):
    ...
    History path (fallback): POST /history/failed/{id}
      — marks the grab as failed and blocklists it; also triggers a
        re-search for the movie, which is acceptable since the release
        is confirmed bad.
    """
    ...
    def blocklist_from_queue(self, queue_id: int) -> bool:
        """
        Remove from Radarr queue and blocklist the release.
    ...
    def blocklist_from_history(self, history_id: int) -> bool:
        """
        Mark history item as failed. Radarr blocklists the release and
        triggers a re-search for the movie — acceptable since the release
        is confirmed bad.
        """
